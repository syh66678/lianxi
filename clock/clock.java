package com.example.android.architecture.blueprints.todoapp.clock;
...
public class clock extends View {
    ...
    public clock(Context context) {
        super(context);
        //obtainStyledAttrs(null); //获取自定义的属性
        //init(); //初始化画笔
    }
    ...
    private void paintPointer(Canvas canvas) {
        ...
        hour = calendar.get(Calendar.HOUR); //时
    ...
    private void paintScale(Canvas canvas) {
        mPaint.setStrokeWidth(SizeUtil.dip2px(getContext(), 1));
        int lineWidth = 0;

        for (int i = 0; i < 60; i++) {
            if (i % 5 == 0) { //整点
                mPaint.setStrokeWidth(SizeUtil.dip2px(getContext(), 1.5f));
                ...
                canvas.save();
                canvas.translate(0, -mRadius + SizeUtil.dip2px(getContext(),15) + lineWidth + (textBound.bottom - textBound.top));
                canvas.rotate(-6 * i);
                mPaint.setStyle(Paint.Style.FILL);
            ...
            canvas.drawLine(0, mRadius - SizeUtil.dip2px(getContext(), 20), 0, mRadius - SizeUtil.dip2px(getContext(), 20) + lineWidth, mPaint);
            canvas.rotate(6);
        }
        ...
        mPaint.setStyle(Paint.Style.FILL);
        canvas.drawLine(0, 0, 450, 450, mPaint);      //绘制直线
        */
        //canvas.restore();
    ...
    //绘制刻度
    private void spaintScale(Canvas canvas) {
        mPaint.setStrokeWidth(SizeUtil.dip2px(getContext(), 1));
        int lineWidth = 0;
        for (int i = 0; i < 60; i++) {
            if (i % 5 == 0) { //整点
                ...
                mPaint.setColor(Color.BLACK);
                canvas.save();
                canvas.translate(0, -mRadius + SizeUtil.dip2px(getContext(),5) + lineWidth + (textBound.bottom - textBound.top));
                canvas.rotate(-6 * i);
                mPaint.setStyle(Paint.Style.FILL);
                canvas.drawText(text, -(textBound.right - textBound.left) / 2,textBound.bottom, mPaint);
                canvas.restore();
            ...
            canvas.drawLine(0, -mRadius + SizeUtil.dip2px(getContext(), 10), 0, -mRadius + SizeUtil.dip2px(getContext(), 10) + lineWidth, mPaint);
            canvas.rotate(6);
        }
        canvas.restore();
    }
    ...
    private void obtainStyledAttrs(AttributeSet attrs) {
        ...
        try {
            ...
            mPadding = array.getDimension(R.styleable.WatchBoard_wb_padding, SizeUtil.dip2px(getContext(), 10));
            mTextSize = array.getDimension(R.styleable.WatchBoard_wb_text_size, SptoPx(16));
            mHourPointWidth = array.getDimension(R.styleable.WatchBoard_wb_hour_pointer_width, SizeUtil.dip2px(getContext(),5));
            mMinutePointWidth = array.getDimension(R.styleable.WatchBoard_wb_minute_pointer_width, SizeUtil.dip2px(getContext(),3));
            mSecondPointWidth = array.getDimension(R.styleable.WatchBoard_wb_second_pointer_width, SizeUtil.dip2px(getContext(),2));
            mPointRadius = (int) array.getDimension(R.styleable.WatchBoard_wb_pointer_corner_radius, SizeUtil.dip2px(getContext(),10));
            mPointEndLength = array.getDimension(R.styleable.WatchBoard_wb_pointer_end_length, SizeUtil.dip2px(getContext(),10));
            ...
            //一旦出现错误全部使用默认值
            mPadding = SizeUtil.dip2px(getContext(),10);
            mTextSize = SptoPx(16);
            mHourPointWidth = SizeUtil.dip2px(getContext(),5);
            mMinutePointWidth = SizeUtil.dip2px(getContext(),3);
            mSecondPointWidth = SizeUtil.dip2px(getContext(),2);
            mPointRadius = (int) SizeUtil.dip2px(getContext(),10);
            mPointEndLength = SizeUtil.dip2px(getContext(),10);
